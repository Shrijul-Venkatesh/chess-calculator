...
move_log_panel_width = (
    210  # May want to adjust this if the board_width/board_height is changed.
)
...
def main():
    ...
    player_clicks = (
        []
    )  # Keeps track of player clicks (two tuples: ex. [(6, 4), (4, 4)])
    game_over = False
        ...
        for event in p.event.get():
            ...
            # Mouse handler
                ...
                if not game_over and human_turn:
                    ...
                    if (
                        square_selected == (row, column) or column >= dimension
                    ):  # User clicks same square or move log
                        ...
                        player_clicks.append(
                            square_selected
                        )  # Appends both 1st and 2nd clicks
...
def animate_move(move, screen, board, clock):
    ...
    for frame in range(frame_count + 1):  # Need +1 to complete the entire animation
        ...
        end_square = p.Rect(
            move.end_column * sq_size, move.end_row * sq_size, sq_size, sq_size
        )
